...
# Ensure repo root is on sys.path so we can import Test.test_API
repo_root = os.path.abspath(os.path.dirname(__file__))
if repo_root not in sys.path:
	sys.path.insert(0, repo_root)

from Extract.extract_api import fetch_open_meteo, fetch_weather
from Test.test_API import main as cli_main
...
def test_cli_main_runs():
	"""
	Simple smoke test: exécute le CLI de test (Test/test_API.py).
	Retourne 0 en cas de succès (API reachable).
	"""
	# Pass an empty argv list so argparse won't consume pytest's args
	rc = cli_main([])
	assert rc == 0
